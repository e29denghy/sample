...
<article class="shell article-page">
    <header class="article-header"><a class="back-link" href="{{ route('articles.index') }}">← 返回文章</a><h1>{{ $revision->title }}</h1><p class="article-lede">{{ $revision->excerpt }}</p><div class="article-card-meta"><time datetime="{{ $article->first_published_at?->toIso8601String() }}">发布于 {{ $article->first_published_at?->format('Y-m-d') }}</time><span>更新于 {{ $article->published_at?->format('Y-m-d') }}</span></div></header>
    <div class="article-body prose">{!! $revision->rendered_html !!}</div>
    <details class="article-revision-note"><summary>版本与资料说明</summary><p>当前为第 {{ $revision->version }} 版，更新于 {{ $article->published_at?->format('Y-m-d') }}。@if($revision->verified_at) 编辑核对日期：{{ $revision->verified_at->format('Y-m-d') }}。@endif</p><p>资料来源、测试条件和未完成的验证，以正文中的具体说明为准。</p></details>
