...
<article class="shell article-page">
    <header class="article-header"><p class="eyebrow">ENGINEERING NOTE · v{{ $revision->version }}</p><h1>{{ $revision->title }}</h1><p class="article-lede">{{ $revision->excerpt }}</p><div class="article-card-meta"><time datetime="{{ $article->published_at?->toIso8601String() }}">发布于 {{ $article->first_published_at?->format('Y-m-d') }}</time><span>更新于 {{ $article->published_at?->format('Y-m-d') }}</span>@if($revision->verified_at)<span>验证于 {{ $revision->verified_at->format('Y-m-d') }}</span>@endif</div></header>
    <div class="article-body prose">{!! $revision->rendered_html !!}</div>
    <footer class="article-footer"><div class="tag-row">@foreach($article->tags as $tag)<a href="{{ route('tags.show', $tag->slug) }}">#{{ $tag->name }}</a>@endforeach</div>@if($article->projects->isNotEmpty())<p>关联项目：@foreach($article->projects as $project)<a href="{{ route('projects.show', $project->slug) }}">{{ $project->name }}</a>@if(!$loop->last)、@endif @endforeach</p>@endif</footer>
</article>
@endsection
